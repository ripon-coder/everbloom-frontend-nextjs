...
export default function ShopPageClient({ initialData, searchParams }: ShopPageClientProps) {
  ...
  const [totalPages, setTotalPages] = useState(initialData.pagination.last_page);
  ...
  const fetchProductsWithParams = useCallback(
    async ({
      ...
      try {
        ...
        const newProducts: Product[] = apiData.products || [];
        const pagination = apiData.pagination || { current_page: 1, last_page: 1 };
  ...
  useEffect(() => {
    ...
    const brandFromUrl = urlSearchParams.get("brand");
    const minP = urlSearchParams.get("min_price");
    const maxP = urlSearchParams.get("max_price");
    ...
    // Only fetch if we don't have initial data or if URL params differ from initial data
    if (initialData.products.length === 0 ||
        categoryFromUrl !== searchParams.category ||
        brandFromUrl !== searchParams.brand) {
      fetchProductsWithParams({
  ...
  }, [urlSearchParams, fetchProductsWithParams, initialData.products.length, searchParams]);

  // --- Fetch Categories & Brands when category changes ---
  useEffect(() => {
